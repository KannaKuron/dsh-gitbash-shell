...
- **④ 卡片显示真实状态(防御性实现,当前不可达)**:从**同一份 peer 快照**读可选的 `pythonBackend`;意图 on + 生效 `node` ⇒ 状态值显示「Python(实验性) · **后端不可用,当前仍为 Node(原因见宿主启动日志)**」并另起一行红字;新增第 7 个词典键 `python.degraded`(**21 门语言**同齐)。字段缺省(旧 peer)⇒ 卡片与 v0.26.0 **逐字节一致**(不显示降级态)。
  - **为什么当前不可达(Lead 裁决 A,2026-09-25)**:浏览器侧只能读 `configForms.get('ptc-cordis').getSnapshot().value`(该行 **Config / settings 表单**),而 `pythonBackend` 是**能力服务**字段(宿主侧,浏览器读不到);ptc 侧**不会**把生效态投影进 settings 表单快照、也不会单方面回写用户意图。因此「意图 on + 生效 node」时:**ptc 卡显示 on + 红字原因,本插件卡只显示 on**(hint 已指向宿主日志)。本插件的降级分支要到对方把 `pythonBackend`(或等价只读派生态)**投影进行 Config 快照**后才会亮起,届时本插件**无需改动**。该后续增强记入最终报告的「已知限制/后续项」,本轮不做。
