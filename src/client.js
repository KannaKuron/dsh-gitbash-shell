...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var zh = {
			...
			"errs.label": "报错路径翻译",
			"errs.hint": "文件工具报错里的 Windows 路径翻回 /c/ 形式并附设备路径引导;关闭则保留原文(便于排障复制)。",
			"split.label": "glob 绝对路径拆分",
			"split.hint": "glob 的绝对 pattern(/c/.../*.md)自动拆为 path + 相对 pattern;关闭则原样传递。",
			"code.label": "run_code 程序内路径",
			"code.hint": "run_code 程序里写的 /c/... 等路径字面量在执行前按同一张挂载表翻译;注释、插值模板、$VAR 与非路径字符串不动,扫描有任何不确定就整份不改。",
		...
		var en = {
			...
			"errs.hint": "Windows paths in file-tool errors rewrite back to /c/ with NUL guidance; off keeps raw text (easier to copy while debugging).",
			...
			"code.hint": "Path literals inside a run_code program are translated through the same mount table before it runs; comments, interpolated templates, $VARs and non-path strings stay untouched, and any uncertainty leaves the whole program as written.",
		...
		var LOCALES = {
			...
			"ar": {
				...
				"errs.hint": "مسارات Windows في أخطاء أدوات الملفات تُعاد بصيغة /c/ مع إرشاد NUL؛ الإيقاف يحتفظ بالنص الأصلي.",
				...
				"code.hint": "تُترجم السلاسل النصية للمسارات المكتوبة داخل برنامج run_code عبر جدول التحميل نفسه قبل التشغيل؛ وتبقى التعليقات والقوالب ذات الإدراج و$VAR والسلاسل غير المسارية كما هي، وأي التباس يُبقي البرنامج كاملًا دون تغيير.",
			...
			"de": {
				...
				"errs.hint": "Windows-Pfade in Dateitool-Fehlern werden zu /c/ zurückgeschrieben; aus erhält den Originaltext.",
				...
				"code.hint": "Pfadliterale in einem run_code-Programm werden vor dem Start über dieselbe Mount-Tabelle übersetzt; Kommentare, interpolierte Templates, $VARs und Nicht-Pfad-Strings bleiben unangetastet, und bei Unklarheit bleibt das ganze Programm unverändert.",
			...
			"fr": {
				...
				"errs.hint": "Les chemins Windows dans les erreurs des outils fichiers sont réécrits en /c/ ; désactivé conserve le texte brut.",
				...
				"code.hint": "Les littéraux de chemin écrits dans un programme run_code sont traduits via la même table de montage avant exécution; commentaires, gabarits interpolés, $VAR et chaînes non-chemin restent intacts, et la moindre incertitude laisse le programme tel quel.",
			...
			"hi": {
				...
				"errs.hint": "फ़ाइल टूल की त्रुटियों में Windows पथ /c/ रूप में लिखे जाते हैं; बंद = मूल पाठ।",
				...
				"code.hint": "run_code प्रोग्राम में लिखे पथ लिटरल चलने से पहले उसी माउंट तालिका से अनूदित होते हैं; टिप्पणियाँ, इंटरपोलेटेड टेम्पलेट, $VAR और गैर-पथ स्ट्रिंग अछूते रहते हैं, और किसी भी अनिश्चितता पर पूरा प्रोग्राम अपरिवर्तित रहता है।",
			...
			"id": {
				...
				"errs.hint": "Path Windows di error alat berkas ditulis balik ke /c/; nonaktif mempertahankan teks asli.",
				...
				"code.hint": "Literal jalur yang ditulis di program run_code diterjemahkan lewat tabel mount yang sama sebelum dijalankan; komentar, template berinterpolasi, $VAR, dan string non-jalur dibiarkan, dan bila ada keraguan seluruh program tidak diubah.",
			...
			"it": {
				...
				"errs.hint": "I percorsi Windows negli errori degli strumenti file tornano in /c/; off mantiene il testo originale.",
				...
				"code.hint": "I letterali di percorso scritti in un programma run_code vengono tradotti con la stessa tabella di mount prima dell'esecuzione; commenti, template interpolati, $VAR e stringhe non di percorso restano intatti e qualsiasi incertezza lascia il programma invariato.",
			...
			"ja": {
				...
				"errs.label": "エラーパス翻訳",
				"errs.hint": "ファイルツールのエラー内の Windows パスを /c/ 形式に書き戻します;オフなら原文維持。",
				"split.label": "glob 絶対パターン分割",
				"split.hint": "絶対 glob パターンを path + 相対パターンに分割;オフならそのまま渡します。",
				"code.label": "run_code プログラム内のパス",
				"code.hint": "run_code プログラム内に書いた /c/... などのパスリテラルは実行前に同じマウント表で変換されます。コメント・補間テンプレート・$VAR・パス以外の文字列は変更せず、解析に不確かさがあればプログラム全体をそのまま実行します。",
			...
			"ko": {
				...
				"errs.label": "오류 경로 번역",
				"errs.hint": "파일 도구 오류의 Windows 경로를 /c/ 형식으로 되돌립니다; 끄면 원문 유지.",
				"split.label": "glob 절대 패턴 분할",
				"split.hint": "절대 glob 패턴을 path + 상대 패턴으로 분할; 끄면 그대로 전달.",
				"code.label": "run_code 프로그램 경로",
				"code.hint": "run_code 프로그램에 쓴 /c/... 경로 리터럴은 실행 전에 같은 마운트 표로 변환됩니다. 주석, 보간 템플릿, $VAR, 경로가 아닌 문자열은 그대로 두고, 스캔이 불확실하면 프로그램 전체를 그대로 둡니다.",
			...
			"nl": {
				...
				"errs.hint": "Windows-paden in fouten van bestandstools keren terug als /c/; uit behoudt de brontekst.",
				...
				"code.hint": "Pad-literalen in een run_code-programma worden vóór uitvoering via dezelfde mounttabel vertaald; opmerkingen, geïnterpoleerde templates, $VAR's en niet-pad-strings blijven ongemoeid, en bij twijfel blijft het hele programma ongewijzigd.",
			...
			"pl": {
				...
				"errs.hint": "Ścieżki Windows w błędach narzędzi plikowych wracają jako /c/; wył. zachowuje oryginalny tekst.",
				...
				"code.hint": "Literały ścieżek w programie run_code są tłumaczone przez tę samą tabelę montowania przed uruchomieniem; komentarze, szablony z interpolacją, $VAR i ciągi niebędące ścieżkami pozostają nietknięte, a każda niepewność pozostawia program bez zmian.",
			...
			"pt": {
				...
				"errs.hint": "Caminhos Windows nos erros das ferramentas voltam como /c/; desligado mantém o texto original.",
				...
				"code.hint": "Literais de caminho escritos num programa run_code são traduzidos pela mesma tabela de montagem antes da execução; comentários, modelos interpolados, $VAR e strings que não são caminhos ficam intactos, e qualquer incerteza deixa o programa como está.",
			...
			"ru": {
				...
				"errs.hint": "Пути Windows в ошибках файловых инструментов возвращаются как /c/; выкл сохраняет исходный текст.",
				...
				"code.hint": "Строковые литералы путей в программе run_code переводятся по той же таблице монтирования до запуска; комментарии, шаблоны с интерполяцией, $VAR и строки-не-пути не трогаются, а при любой неопределённости программа остаётся без изменений.",
			...
			"sv": {
				...
				"errs.hint": "Windows-sökvägar i filverktygens fel skrivs tillbaka som /c/; av behåller originaltexten.",
				...
				"code.hint": "Sökvägslitteraler i ett run_code-program översätts via samma monteringstabell före körning; kommentarer, interpolerade mallar, $VAR och strängar som inte är sökvägar lämnas orörda, och vid minsta osäkerhet lämnas hela programmet oförändrat.",
			...
			"th": {
				...
				"errs.hint": "เส้นทาง Windows ในข้อผิดพลาดของเครื่องมือไฟล์เขียนกลับเป็น /c/; ปิดคงข้อความต้นฉบับ",
				...
				"code.hint": "สตริงพาธที่เขียนในโปรแกรม run_code จะถูกแปลผ่านตาราง mount เดียวกันก่อนรัน ความคิดเห็น เทมเพลตที่มีการแทรก $VAR และสตริงที่ไม่ใช่พาธจะไม่ถูกแตะ และหากไม่แน่ใจทั้งโปรแกรมจะไม่ถูกแก้ไข",
			...
			"tr": {
				...
				"errs.hint": "Dosya aracı hatalarındaki Windows yolları /c/ olarak geri yazılır; kapalı özgün metni korur.",
				...
				"code.hint": "Bir run_code programında yazılan yol dizeleri çalıştırılmadan önce aynı bağlama tablosuyla çevrilir; yorumlar, enterpolasyonlu şablonlar, $VAR ve yol olmayan dizeler olduğu gibi kalır, herhangi bir belirsizlikte programın tamamı değişmeden bırakılır.",
			...
			"vi": {
				...
				"errs.hint": "Đường dẫn Windows trong lỗi công cụ tệp viết lại thành /c/; tắt giữ nguyên văn bản gốc.",
				...
				"code.hint": "Chuỗi đường dẫn viết trong chương trình run_code được dịch qua cùng bảng mount trước khi chạy; chú thích, template nội suy, $VAR và chuỗi không phải đường dẫn được giữ nguyên, và mọi điểm không chắc chắn sẽ khiến cả chương trình không bị sửa.",
			...
			"zh-hk": {
				...
				"errs.label": "報錯路徑翻譯",
				"errs.hint": "檔案工具報錯入面嘅 Windows 路徑翻回 /c/ 形式;關閉就保留原文。",
				"split.label": "glob 絕對路徑拆分",
				"split.hint": "glob 嘅絕對 pattern 自動拆做 path + 相對 pattern;關閉就原樣傳遞。",
				"code.label": "run_code 程式內路徑",
				"code.hint": "run_code 程式內寫的 /c/... 路徑字面量會在執行前按同一張掛載表翻譯;註解、插值模板、$VAR 與非路徑字串不動,掃描有任何不確定就整份不改。",
			...
			"zh-tw": {
				...
				"errs.label": "報錯路徑翻譯",
				"errs.hint": "檔案工具報錯裡的 Windows 路徑翻回 /c/ 形式;關閉則保留原文。",
				"split.label": "glob 絕對路徑拆分",
