...
4f. **run_code 的实验性 Python 后端开关(v0.26.0)**:dsh 的实验性 CPython PTC 后端
     ...
     - **互斥的判据是「生效态」,不是「意图」(v0.26.1,重要)**:`pythonBackendActive(capability)`
       ...
       **不会**把生效态投影进 settings 表单、也不回写用户意图,所以该降级分支**当前不可达**
       (防御性实现):意图 on + 生效 node 时,ptc 卡显示 on + 红字原因,本插件卡只显示 on(hint 指向
       宿主日志)。后续增强 = 让对方用**加法只读字段**把 `pythonBackend`/`pythonIssue` 投影进行
       Config 快照(不动用户意图字段);那一刻本插件**无需改动**即亮起。**不要把"已生效"写进面向
       用户的文案或文档。**
